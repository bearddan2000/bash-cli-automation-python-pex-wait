...
function start-up(){
    ...
    case $docker_img_name in
      *bottle-ssl* ) 
        docker run --rm -p 443:443 $docker_img_name /bin/bash -c "pants run :app" ;;
      *bottle* ) 
        docker run --rm -p 80:8000 $docker_img_name /bin/bash -c "pants run :app" ;;
      *cherrypy* ) 
        docker run --rm -p 80:8080 $docker_img_name /bin/bash -c "pants run :app" ;;
      *dash* ) 
        docker run --rm -p 80:8050 $docker_img_name /bin/bash -c "pants run :app" ;;
      *falcon* ) 
        docker run --rm -p 80:8000 $docker_img_name /bin/bash -c "pants run :app" ;;
      *fastapi* ) 
        docker run --rm -p 80:8000 $docker_img_name /bin/bash -c "pants run :app" ;;
      *flask-ssl* ) 
        docker run --rm -p 443:443 $docker_img_name /bin/bash -c "pants run :app" ;;
      *flask* ) 
        docker run --rm -p 80:5000 $docker_img_name /bin/bash -c "pants run :app" ;;
      *pyramid* ) 
        docker run --rm -p 80:6543 $docker_img_name /bin/bash -c "pants run :app" ;;
      *tornado* ) 
        docker run --rm -p 80:8000 $docker_img_name /bin/bash -c "pants run :app" ;;
      /? ) 
